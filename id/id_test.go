...
func TestConstructors(t *testing.T) {
	tests := []struct {
		...
		{"EventTypeID", id.NewEventTypeID, "evtype_"},
		{"EndpointID", id.NewEndpointID, "ep_"},
		{"EventID", id.NewEventID, "evt_"},
		{"DeliveryID", id.NewDeliveryID, "del_"},
		{"DLQID", id.NewDLQID, "dlq_"},
		{"SecretID", id.NewSecretID, "whsec_"},
	}
...
func TestNew(t *testing.T) {
	i := id.New(id.PrefixEvent)
	if i.IsNil() {
		t.Fatal("expected non-nil ID")
	}
	if i.Prefix() != id.PrefixEvent {
		t.Errorf("expected prefix %q, got %q", id.PrefixEvent, i.Prefix())
	}
}
...
func TestParseRoundTrip(t *testing.T) {
	tests := []struct {
		...
		{"EventTypeID", id.NewEventTypeID, id.ParseEventTypeID},
		{"EndpointID", id.NewEndpointID, id.ParseEndpointID},
...
func TestCrossTypeRejection(t *testing.T) {
	tests := []struct {
		...
		{
			"ParseEventTypeID rejects ep_ prefix",
			id.NewEndpointID().String(),
			id.ParseEventTypeID,
		},
		{
			"ParseEndpointID rejects evt_ prefix",
			id.NewEventID().String(),
			id.ParseEndpointID,
		},
		{
			"ParseEventID rejects del_ prefix",
			id.NewDeliveryID().String(),
			id.ParseEventID,
		},
		{
			"ParseDeliveryID rejects dlq_ prefix",
			id.NewDLQID().String(),
			id.ParseDeliveryID,
		},
		{
			"ParseDLQID rejects evtype_ prefix",
			id.NewEventTypeID().String(),
			id.ParseDLQID,
		},
	}
...
func TestParseWithPrefix(t *testing.T) {
	i := id.NewEventID()
	parsed, err := id.ParseWithPrefix(i.String(), id.PrefixEvent)
	if err != nil {
	...
	_, err = id.ParseWithPrefix(i.String(), id.PrefixEndpoint)
	if err == nil {
...
func TestUniqueness(t *testing.T) {
	...
	if a.String() == b.String() {
		t.Errorf("two consecutive NewEventID() calls returned the same ID: %q", a.String())
	}
}

func TestBSONRoundTrip(t *testing.T) {
	original := id.NewEventTypeID()
